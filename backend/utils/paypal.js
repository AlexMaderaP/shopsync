...
dotenv.config();
const { PAYPAL_CLIENT_ID, PAYPAL_APP_SECRET, PAYPAL_API_URI } = process.env;
...
async function getPayPalAccessToken() {
  ...
  const url = `${PAYPAL_API_URI}/v1/oauth2/token`;
...
export async function verifyPayPalPayment(paypalTransactionId) {
  ...
  const paypalResponse = await fetch(
    `${PAYPAL_API_URI}/v2/checkout/orders/${paypalTransactionId}`,
    {
      headers: {
